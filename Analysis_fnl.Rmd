...
We begin with these 3 raw datasets.

### 1. Congressional elections vote totals (1976-2020-house.csv

### 2. Redistricting data (StatesAndCyclesData.csv)

### 3. Voter turnout data (VoteAndRegByState2020.xlsx) 

After cleaning and merging the three above, we get the final dataset:

#### Final datasets (analysis_data.csv)
...
stargazer(turn_reg_all, Midterm, Presidential, column.labels = c("Overall", "Mid", "Pres"), type ="text")
...
plm1 <- plm(turnout_perc ~ partisan + mid_term, data=redist2, index=c("State", "Year"),
...
### Democratic vote share margins - Lineplot

We plot lineplots of average voteshare margins through time, for each type of redistricting policy. We observe that legislature and politician commission states have the highest vote margins, and that margins have been creeping upwards throughout the period.
...
### Democratic vote margin - scatterplots

The visualization filters the results to elections in states with more than one district, and also excludes races where a Democrat or Republican received zero votes (i.e. they were not on the ballot). The following scatterplots are faceted by states' redistricting policies, and plot Democratic vote counts on the X-axis and Republican vote counts on the Y-axis. The points represents district-elections.
\
We see that in the facet 'Legislature', there is a larger cluster of points away from the 45-degree diagonal line, which indicates points where both parties get equal votes. 
...
### Democratic vote margin - Histogram

The histogram depicts the distribution of district-wise democratic vote margins in all elections from 2000-2020, for states with legislative redistricting. The pattern from the above scatterplot becomes clearer. We see that it is a bimodal distribution, indicating that there are relatively fewer districts in the middle of the distribution, around 0 percent. These low margins would indicate very closely fought races. It implies that there are more elections which are being won by large margins, as high as 50%.
