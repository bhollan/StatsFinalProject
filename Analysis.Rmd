...
redist2 <- redist %>% filter(Drawn.by!="n/a") %>%
  mutate(turnout_prop = turnout_perc / 100) %>%
  filter(Seats != 1,
         Year < 2022)
...
aov_results <- aov(turnout_perc ~ Drawn.by, data = redist2)
...
turnout_regression <- lm(turnout_perc ~ Drawn.by+mid_term,
                         data = redist2)
...
stargazer(turnout_regression, mid, pres, type ="text")
...
where the units and time periods have different characteristics that may affect the response variable, and where the observations are correlated over time.

The fixed effects for the state and year variables capture the unobserved heterogeneity and autocorrelation in the data
