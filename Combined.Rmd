...
elec <- read.csv("/Users/scrubscrub/stats1/StatsFinalProject/1976-2020-house.csv", stringsAsFactors = FALSE)
head(elec)

redistricting_raw <- read.csv("/Users/scrubscrub/stats1/StatsFinalProject/StatesAndCyclesData.csv", stringsAsFactors = FALSE)
head(redistricting_raw)
...
census_api_key("858d681fdc0a9574c3a9fab6d88774cb58fba2f2")
...
election_map<-merge(allst2, df, by.x = "NAME", by.y = "state" )

election_map %>% mutate(dpct = demv/turnout) %>%
  ggplot() + 
  geom_sf(aes(color = dpct, fill = dpct))+
  labs(title = "Democratic Votes by State")+
...
```{r , echo=TRUE, message =FALSE, warning=FALSE}
## Republican votes
election_map %>% mutate(rpct = repv/turnout) %>%
  ggplot(aes(fill = rpct)) + 
  geom_sf() +
  scale_fill_viridis_c(option = "red",begin = 0.1)+
  labs(title = "Republican Votes by State")+
  theme(plot.title = element_text(hjust=0.5, face="bold"), #moves title to the middle of graph, bolds it
          text= element_text(family= "Times", size = ))
```
...
## This only shows 4 institutions
...
library(maps)
usa <- map_data("state")
usa$region <- capitalize(usa$region)
...
election_map2 %>%
  ...
  coord_map("polyconic")+
  labs(title = "Redistricting Institution by State (2000-2010)")+
...
election_map2 %>%
  ...
  geom_polygon(colour = "black") +
  coord_map("polyconic")+
  labs(title = "Redistricting Institution by State (2010-2020)")+
  theme(plot.title =element_text(hjust=0.5, face="bold"), 
          text= element_text(family= "Times", size = ))
...
##Average Turnout for Each Institution
...
``` {r, echo=TRUE, message =FALSE, warning=FALSE}
#group by institution and get average turnout, dataframe = df
institution_mean <- df %>%
  group_by(Institution)%>%
  summarize(average_turnout = mean(x= turnout2))

institution_mean

ggplot(institution_mean, aes(x = Institution, y = average_turnout))+ 
  geom_col()+
  labs(title = "Average Voter Turnout Per Institution", 
       y= "Average Turnout") +
  theme_minimal() +
  theme(plot.title =element_text(hjust=0.5, face="bold"), #moves title to the middle of graph, bolds it
...
## Dem and Rep vote percentage over time
``` {r, echo=TRUE, message =FALSE, warning=FALSE}
## create turnout_df
...
count %>% 
  ...
  geom_text(aes(x=1.4, label=percent(prop)), position=position_stack(vjust=0.5), size=2.5) +
  labs(x = NULL, y = NULL) +
  scale_fill_brewer(palette="Pastel2")+
  labs(title = "Institution Proportion (2000-2010")+
  theme(plot.title =element_text(hjust=0.5, face="bold"), 
          text= element_text(family= "Times", size = ))
...
count %>% 
  ...
  labs(x = NULL, y = NULL) +
  scale_fill_brewer(palette="Pastel2")+
  labs(title = "Institution Proportion (2010-2020)")+
  theme(plot.title =element_text(hjust=0.5, face="bold"), 
          text= element_text(family= "Times", size = ))

```

##Effect of institution on the share of party votes
-make a similar graph for the effect of each institution on percentage of registered voters who voted
```{r,echo=TRUE, message =FALSE, warning=FALSE}

## The effect of each institution on the percentage of registered voters who voted
# df %>%
#   #get data on registered voters
#   mutate(voterpct = turnout/REGISTEREDVOTERS) %>%
#   ggplot(aes(x= year, y = voterpct, colour = Drawn.by)) +
#       geom_point(size = 1) +
#       labs(title="Title")
...
df %>%
  ...
  ggplot(aes(x= year, y = dpct, colour = Drawn.by)) +
      geom_point(size = 1) +
      labs(title = "Share of Democrat Votes per Redistricting Commission",
           x = "Year", y = "Democrat Vote Percentage")+
  theme(plot.title =element_text(hjust=0.5, face="bold"), 
          text= element_text(family= "Times", size = ))
...
df %>%
  ...
  ggplot(aes(x= year, y = rpct, colour = Drawn.by)) +
      geom_point(size = 1) +
      labs(title = "Share of Republican Votes per Redistricting Commission",
           x = "Year", y = "Republican Vote Percentage")+
  theme(plot.title =element_text(hjust=0.5, face="bold"), 
          text= element_text(family= "Times", size = ))
