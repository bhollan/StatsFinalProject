---
title: "Redistricting project STATS PPOL 560"
output:
  html_document:
    df_print: paged
---
...
## Raw datasets:
   1976-2020-house.csv
   StatesAndCyclesData.csv
   VoteAndRegByState2020.xlsx
   VoteAndRegByStateHistory_prez_reg.xlsx
   VoteAndRegByStateHistory_prez_vot.xlsx
   VoteAndRegByStateHistory_congress.xlsx
   
# Part-2 Analysis
After cleaning and merging the above, we use two different dataframes for state-level and district-level analysis.
We have uploaded:
votes_redistricting.csv

This dataframe will run some of the analysis here, but not all, due to some of the unit-level incongruities.
All our source data is in our repository on GitHub here:
https://github.com/bhollan/StatsFinalProject
...
This is the data set which is the output of the data cleaning.


```{r , echo=TRUE, message=FALSE, warning=FALSE}

# All the necessary data to build our project is available publically on GitHub here:
# https://github.com/bhollan/StatsFinalProject
# 
votes <- read.csv("./source_data/votes_redistricting.csv", stringsAsFactors = FALSE)
...
# This code block outputs lots of noise about "New names:" as an artifact of reading `.xlsx` cells with no header row.
# There was no built-in or simple way to suppress this output without an insanely complicated wrapper function, so we left it.
```
...
# This API key is not an important secret. We had the `keyring` package working, but figured you didn't want to create your own API key to test things. We will likely delete this after grading is complete.
...
inst_lineplt <- election_map %>% 
  group_by(Drawn.by, Year) %>% 
  summarise(avg_turnout = mean(turnout_perc)) %>% 
  filter(Drawn.by!="n/a")

lesscom <- inst_lineplt %>% 
  filter(Drawn.by %in% c("State court", "Federal court", "Legislature", "Independent commission"))
...
votes %>% filter(Drawn.by=="Legislature") %>%
  ...
  ggplot(aes(x = dem_margin)) +
    ...
    labs(title = "Democratic vote margin (Legislature states' races)",
         x = "Margin (percent)",
         y = "Frequency")
```
